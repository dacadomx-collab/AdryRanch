...
| v1.4 | 2026-06-23 | Corregido bug real en `.htaccess` (regex `^/api/` no coincidía con la URI local `/AdryRanch/api/...`, causaba 403 en todos los endpoints). Topbar del Dashboard: logo institucional `login.png` reemplaza el ícono lineal dorado. Nuevo Conmutador de Iluminación interno del Dashboard (modo claro "broken white", independiente del toggle público). |
| v1.5 | 2026-06-23 | Directiva Suprema de Usabilidad Móvil: Reingeniería mobile-first total del Dashboard — menú hamburguesa exclusivo del panel (`#botonHamburguesaDashboard`), tablas de corredores/usuarios mutadas a tarjetas apilables en `<=800px` vía `data-label`, formulario de tareas y tablero apilados, objetivos táctiles `>=44px`. Corregido bug real de la clase `.oculto` (no ocultaba los íconos del nuevo hamburguesa por scope incorrecto). Verificado con Playwright en mobile (375px) y desktop (1280px), en ambos modos de iluminación. |
| v1.6 | 2026-06-23 | **Detención forense del error 500 en producción** (`login_admin.php` → "Configuración no encontrada"): nuevo `api/env_loader.php` centraliza la carga del `.env` con escaneo defensivo multirruta, eliminando la duplicación de `parse_ini_file()` en `conexion.php`/`test_conexiones.php`/`crear_usuarios_iniciales.php`. Datos reales e inmutables de "Ediciones Anteriores" (14-jun-2026, 6:30 am, de 6 a 8 km, 35 corredores) reemplazando todos los placeholders. Galería con Lightbox nativo (vanilla JS, sin librerías) — Hero/video intacto. Confirmado sin cambios: `.htaccess`, mobile-first del Dashboard y Conmutador de Iluminación interno, ya conformes desde v1.4-v1.5. |
